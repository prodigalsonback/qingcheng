...
public class SpecServiceImpl implements SpecService {
    ...
    public void saveSpecListTORedis() {
        List<Map> allSpecLists = specMapper.findAllSpecList();
        //clean
        for(Map allSpecList:allSpecLists) {
            if (allSpecList.get("categoryName") != null) {
                redisTemplate.boundHashOps(CacheKey.SPEC_LIST).delete(allSpecList.get("categoryName"));
            }
        }
        ...
        for(Map allSpecList:allSpecLists) {
            if (allSpecList.get("categoryName") != null) {
                List<Map> specList;
                if (redisTemplate.boundHashOps(CacheKey.SPEC_LIST).hasKey(allSpecList.get("categoryName"))) {
                    specList = (List<Map>) redisTemplate.boundHashOps(CacheKey.SPEC_LIST).get(allSpecList.get("categoryName"));
                } else {
                    specList = new LinkedList<>();
                }
                Map tmp = new HashMap();
                tmp.put("name", allSpecList.get("specName"));
                tmp.put("options", allSpecList.get("specOptions"));
                specList.add(tmp);
                redisTemplate.boundHashOps(CacheKey.SPEC_LIST).put(allSpecList.get("categoryName"), specList);
            }
        }
    }
