...
public class ItemController {
    ...
    public String createPage(String spuid){
	//public void createPage(String spuId){
        ...
        for(Sku sku:skuList){
            File fileExist= new File(pagePath+"/"+sku.getId()+".html");
            if(fileExist.exists()){
                return "redirect:"+"/itemtest/"+sku.getId()+".html";
            }
        ...
        for(Sku sku:skuList){
            ...
            for(String key :specMap.keySet()  ){  //循环规格
                ...
                for(String value:list){
                    ...
                    if(specItems.get(key)==null?false:specItems.get(key).equals(value) ){  // 如果和当前sku的规格相同，就是选中
        ...
        /*File fileExist= new File(pagePath+"/"+skuList.get(0).getId()+".html");
        while(true){
            if(fileExist.exists()){
                //sleep(3000);
                return pagePath+"/"+skuList.get(0).getId()+".html";
            }
        }*/
        return "redirect:"+"/itemtest/"+skuList.get(0).getId()+".html";
        //return pagePath+"/"+skuList.get(0).getId()+".html";"redirect:";
