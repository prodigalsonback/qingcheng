...
public class CartServiceImpl implements CartService {
    ...
    public void deleteCheckedCart(String username) {
       ...
       //为兼容jdk8以下版本
       List<Map<String, Object>> beforeCartList=findCartList(username);
       List<Map<String, Object>> cartList=new LinkedList<>();
       for(Map<String, Object> cart:beforeCartList){
           if((boolean)cart.get("checked")==false){
               cartList.add(cart);
           }
       }
       redisTemplate.boundHashOps(CacheKey.CART_LIST).put(username,cartList);
    ...
    public int preferential(String username) {
        ...
        for(OrderItem orderItem:orderItemList){
            if(cartMap.get(orderItem.getCategoryId3())==null){
                cartMap.put(orderItem.getCategoryId3(),orderItem.getMoney());
            }else {
                int tmpMoney=cartMap.get(orderItem.getCategoryId3());
                cartMap.put(orderItem.getCategoryId3(),tmpMoney+orderItem.getMoney());
            }
        }

        int allPreMoney=0;
        for(Integer categoryId:  cartMap.keySet()){
            int preMoney = preferentialService.findPreMoneyByCategoryId(categoryId,cartMap.get(categoryId));//获取优惠金额
            allPreMoney+= preMoney;
        }
        return allPreMoney;
    }
