...
public class OrderServiceImpl implements OrderService {
    ...
    public Map<String,Object> add(Order order) {
        ...
        for(Map<String, Object> cart:cartList){
            if((boolean)cart.get("checked")==true){
                orderItemList.add((OrderItem)cart.get("item"));
            }
        }
        //2.扣减库存
    ...
    public OrderPairs findOrderPairs(String id) {
        ...
        orderPairs.setOrder(order);
        orderPairs.setOrderItems(orderItems);
        return orderPairs;
    }
    ...
    public void merge(String id1, String id2) {
        ...
        orderItemTmp.setOrderId(id1);
        Example example=new Example(OrderItem.class);
        Example.Criteria criteria = example.createCriteria();
        criteria.andEqualTo("orderId",id2);
        orderItemMapper.updateByExampleSelective(orderItemTmp,example);
        //金额和数目合并
        int totalMoney=order1.getTotalMoney()+order2.getTotalMoney();
        int totalNum=order1.getTotalNum()+order2.getTotalNum();
        order1.setTotalMoney(totalMoney);
        order1.setTotalNum(totalNum);
        ...
        orderLog1.setOperater("system");// 系统
        orderLog1.setOperateTime(new Date());//当前日期
        orderLog1.setPayStatus(order1.getPayStatus());
        orderLog1.setConsignStatus(order1.getConsignStatus());
        orderLog1.setRemarks("将订单号为"+id2+"的订单合并");
        orderLog1.setOrderId(Long.parseLong(order1.getId()));
        orderLogMapper.insert(orderLog1);
        OrderLog orderLog2=new OrderLog();
        orderLog2.setId(idWorker.nextId()+"");
        orderLog2.setOperater("system");// 系统
        orderLog2.setOperateTime(new Date());//当前日期
        orderLog2.setPayStatus(order2.getPayStatus());
        orderLog2.setConsignStatus(order2.getConsignStatus());
        orderLog2.setRemarks("和订单号为"+id1+"的订单合并");
        orderLog2.setOrderId(Long.parseLong(order2.getId()));
    ...
    public void split(List<OrderItem> orderItems) {
        ...
        for(OrderItem orderItem:orderItems){
            if(!(orderItem.getId()==null||orderItem.getNum()==null||orderItem.getNum()==0)) {
                ...
                if (orderItem1.getNum() < orderItem.getNum()){
                    orderMapper.deleteByPrimaryKey(order);
                    throw new RuntimeException("拆分数量超过原数量");
                }else if(orderItem1.getNum() == orderItem.getNum()){
                    numTmp+=orderItem.getNum();
                    moneyTmp+=orderItem.getNum()*orderItem1.getPrice();
                    orderItem1.setOrderId(idTmp+"");
                    orderItemMapper.updateByPrimaryKeySelective(orderItem1);
                }else{
                    numTmp+=orderItem.getNum();
                    moneyTmp+=orderItem.getNum()*orderItem1.getPrice();
                    int n=orderItem1.getNum()-orderItem.getNum();
                    int m=orderItem1.getMoney()-orderItem.getNum()*orderItem1.getPrice();
                    orderItem1.setNum(n);
                    orderItem1.setMoney(m);
                    orderItemMapper.updateByPrimaryKeySelective(orderItem1);
                    orderItem1.setId(idWorker.nextId()+"");
                    orderItem1.setOrderId(idTmp+"");
                    orderItem1.setNum(orderItem.getNum());
                    orderItem1.setMoney(orderItem.getNum()*orderItem1.getPrice());
                    orderItemMapper.insert(orderItem1);
                }
            }
        }
        String oldId=order.getId();
        if(order.getTotalNum()!=null&&order.getTotalNum()==numTmp){
            order.setIsDelete("1");
        }else{
            if(order.getTotalNum()!=null){
            order.setTotalNum(order.getTotalNum()-numTmp);}
            if(order.getTotalMoney()!=null){
            order.setTotalMoney(order.getTotalMoney()-moneyTmp);}
        }
        order.setUpdateTime(new Date());
        orderMapper.updateByPrimaryKeySelective(order);
        order.setIsDelete("0");
        order.setId(idTmp+"");
        order.setTotalNum(numTmp);
        order.setTotalMoney(moneyTmp);
        orderMapper.updateByPrimaryKeySelective(order);
        //日志
        OrderLog orderLog1=new OrderLog();
        orderLog1.setId(idWorker.nextId()+"");
        orderLog1.setOperater("system");// 系统
        orderLog1.setOperateTime(new Date());//当前日期
        orderLog1.setRemarks("订单拆分");
        orderLog1.setOrderId(Long.parseLong(oldId));
        orderLogMapper.insertSelective(orderLog1);
        OrderLog orderLog2=new OrderLog();
        orderLog2.setId(idWorker.nextId()+"");
        orderLog2.setOperater("system");// 系统
        orderLog2.setOperateTime(new Date());//当前日期
        orderLog2.setRemarks("订单拆分");
        orderLog2.setOrderId(Long.parseLong(order.getId()));
